...
import { 
  Image as ImageIcon, Upload, Loader2, AlertTriangle, CheckCircle2
} from 'lucide-react';
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const mockPatient: PatientData = {
        ...
        bloodType: 'Unknown', history: 'Radiology Scan requested.', meds: '', allergies: '',
        smoking: 'None', alcohol: 'None', activity: '', complaints: 'Analyzing radiological findings.',
    ...
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 pb-20 animate-fade-in">
      <div className="bg-white p-8 md:p-12 rounded-[40px] border border-slate-100 shadow-xl space-y-8">
        <h3 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3">
          ...
          {t.radiologyIntake || 'Radiology Intake'}
        </h3>
        <div className={`border-2 border-dashed rounded-[40px] h-96 flex flex-col items-center justify-center transition-all relative overflow-hidden group ${image ? 'border-blue-500 bg-blue-50/10' : 'border-slate-200 hover:bg-slate-50'}`}>
          {image ? (
            <>
              <img src={image} className="h-full w-full object-contain p-8" alt="Scan" />
            ...
            <label className="cursor-pointer text-center p-10 w-full h-full flex flex-col items-center justify-center">
              ...
              <span className="font-black text-slate-700 block uppercase tracking-widest text-xs">{t.uploadRadiograph || 'Upload Radiograph'}</span>
              <p className="text-[10px] text-slate-400 mt-2">{t.scanSupport || 'Scan supported'}</p>
        ...
        <button 
          ...
          className="w-full bg-slate-900 text-white py-6 rounded-3xl font-black uppercase tracking-[0.2em] text-xs disabled:opacity-50 hover:bg-blue-600 transition-all shadow-xl flex items-center justify-center gap-4"
        >
          {loading ? <Loader2 className="animate-spin" /> : <CheckCircle2 size={18} />}
          {loading ? (t.processingVision || 'Processing...') : (t.initiateScan || 'Initiate Scan')}
        </button>
      </div>
      ...
      <div className="space-y-8">
        {!analysis ? (
          <div className="bg-slate-50 border border-slate-100 rounded-[40px] p-20 text-center flex flex-col items-center justify-center h-full min-h-[400px]">
            ...
            <p className="font-black uppercase tracking-[0.2em] text-[10px] text-slate-400">{t.waitingInput || 'Waiting'}</p>
          </div>
        ) : (
          <div className="bg-white p-10 md:p-12 rounded-[40px] border border-slate-100 shadow-2xl space-y-10 animate-fade-in">
            <h4 className="text-3xl font-black text-slate-800 tracking-tight">{analysis.mainDiagnosis}</h4>
